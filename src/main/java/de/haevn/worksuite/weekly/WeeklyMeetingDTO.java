...
public record WeeklyMeetingDTO(UUID id, String title, String summary, List<DaySummaryDTO> daySummaries,
                               Instant createdAt) {
    ...
    public WeeklyMeeting toModel() {
        ...
        if (this.daySummaries() != null) {
            this.daySummaries().stream().map(DaySummaryDTO::toModel).forEach(weeklyMeeting::addDaySummary);
        }
